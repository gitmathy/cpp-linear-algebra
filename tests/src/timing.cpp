...
///
/// @file tests/src/timing.cpp
/// @brief Implementation of time keeping
...
void time_report::report()
{
    ...
    std::size_t max_descr_width =
        std::max_element(p_timings.begin(), p_timings.end(), [](const auto &a, const auto &b) {
    ...
    stringstr << "| " << std::left << std::setw(max_descr_width) << "description"
              << " | " << std::left << std::setw(max_digits_calls) << '#' << " | " << std::left
              << std::setw(13) << "duration [s]"
    ...
    for (auto it = p_timings.begin(); it != p_timings.end(); ++it) {
        stringstr << "| " << std::left << std::setw(max_descr_width) << it->first << " | "
                  << std::right << std::setw(max_digits_calls) << it->second.calls << " | "
                  << std::right << std::setw(13) << std::setprecision(4) << std::fixed
                  << it->second.duration.count() << " | " << std::setw(16) << std::setprecision(4)
                  << std::fixed << (it->second.duration.count() / it->second.calls) << " |\n";
    }
    for (std::size_t i = 0; i < max_descr_width + 3; ++i)
        stringstr << '-';
    stringstr << '|';
    for (std::size_t i = 0; i < max_digits_calls + 2; ++i)
        stringstr << '-';
    stringstr << "|---------------|-------------------" << std::endl;
    stringstr << "| " << std::left << std::setw(max_descr_width) << "Overall"
              << " | " << std::right << std::setw(max_digits_calls) << "1"
              << " | " << std::right << std::setw(13) << std::setprecision(4) << std::fixed
              << total_elapsed_seconds.count() << " |       N/A        |";
